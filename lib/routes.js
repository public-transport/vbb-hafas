...
const parsePart = (p) => {
	let r = {
		...
		transport: util.routes.legs.types.parse(p.type).type
	}

	if (p.Notes) r.notes = util.routes.legs.notes.parse(p.Notes)
	if (p.transport === 'public') {
		r.type = util.products.parseCategory(p.Product.catCode).type
		if (r.type === util.products.express.type)
			r.line = p.Product.line // fixes #8
...
const parseTickets = (data) => {
	if (!data) return null
	try { return util.routes.fares.parse(data.fareSetItem[0].fareItem[0].ticket) }
	catch (e) { return null }
}

const parseRoute = (route) => ({
	duration: util.duration.parse(route.duration),
	parts:    (route.LegList || []).map(parsePart),
...
const onSuccess = (data) => {
	...
	results.concat(data.Trip.map(parseRoute))
}
