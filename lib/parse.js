...
const location = (l) => {
	...
	if ('pCls' in l) result.products = util.products.parseBitmask(l.pCls)
	return result
}
...
const part = (s, p, r, c) => (pt) => {
	const result = {
		...
		, end:       new Date(dateTime(c.date, pt.arr.aTimeR || pt.arr.aTimeS))
	}
...
const departure = (s, p, r) => (d) => {
	...
	if (d.stbStop.dTimeR && d.stbStop.dTimeS) result.delay =
		dateTime(d.date, d.stbStop.dTimeR) - dateTime(d.date, d.stbStop.dTimeS)
	return result
}
