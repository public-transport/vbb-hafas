...
const location = (l) => ({
	...
	, id:        parseInt(l.extId)
	, name:      l.name
	, latitude:  l.crd.x / 1000000
	, longitude: l.crd.y / 1000000
})
...
const stop = (s, p, r, c) => (st) => {
	...
	if (st.aTimeS) result.arrival =
		new Date(dateTime(c.date, st.aTimeS))
	if (st.dTimeS) result.departure =
		new Date(dateTime(c.date, st.dTimeS))
	return result
}
...
const part = (s, p, r, c) => (pt) => {
	const result = {
		...
		, start:     new Date(dateTime(c.date, pt.dep.dTimeS))
		, end:       new Date(dateTime(c.date, pt.arr.aTimeS))
		, product:   p[parseInt(pt.jny.prodX)]
		, direction: pt.jny.dirTxt // todo: parse this
		, passed:    pt.jny.stopL ? pt.jny.stopL.map(stop(s, p, r, c)) : null
	}
...
const departure = (s, p, r) => (d) => {
	const result = {
		...
		, when:      new Date(dateTime(d.date, d.stbStop.dTimeS))
