...
const addTransferInfoToJourney = (j) => {
	...
	for (let i = 0; i < (j.legs.length - 1); i++) {
		...
		if (!fromL.line || !fromL.line.public || !fromL.stopovers) continue
		const toL = j.legs[i + 1]
		if (!toL.line || !toL.line.public || !toL.stopovers) continue

		const fromS = merged[fromL.destination.id]
		if (!fromS) continue
		const fromT = fromL.arrivalPlatform

		const fromIndex = fromL.stopovers.findIndex((st) => {
			return st.stop && st.stop.id === fromL.destination.id
		})
		let prev = fromL.stopovers[fromIndex - 1]
		prev = prev && prev.stop && prev.stop.id
		if (!prev) continue
		...
		const toIndex = toL.stopovers.findIndex((st) => {
			return st.stop && st.stop.id === toL.origin.id
		})
		let next = toL.stopovers[toIndex + 1]
		next = next && next.stop && next.stop.id
		if (!next) continue
